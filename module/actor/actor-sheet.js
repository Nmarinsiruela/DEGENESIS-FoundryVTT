...
export class DegenesisActorSheet extends ActorSheet {
    ...
    _onComplicationEdit(event) {
        ...
        this.actor.createEmbeddedDocuments("Item", [{ type: "complication", name: event.target.value }])
        let item = this.actor.items.get(itemId);
        let target = $(event.currentTarget).attr("data-target")
    ...
    async _onAggregateClick(event) {
        ...
        if (group == "ammunition") {
            ...
            let ammo = await this.actor.deleteEmbeddedDocuments("Item", ids).map(i => i.data)
